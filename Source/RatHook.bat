...
"%GoTools%Goasm\GoAsm.exe" /b /ni /fo "%~2\%~n1.obj" %1
if errorlevel 1 goto errObj

echo.Step #(2/3) : %~n1.obj LIYNKING to %~n1.dll
"%GoTools%Golink\GoLink.exe" /ni /dll /entry RatHookEntryPoint "%~2\%~n1.obj" user32.dll kernel32.dll gdi32.dll ole32.dll
if errorlevel 1 goto errDll

echo.Step #(3/3) : %~n1.def XPORTING to %~n1.lib
"%MSVSBIN%lib.exe" /machine:x86 /def:"%~dpn1.def" /out:"%~2%~n1.lib"
