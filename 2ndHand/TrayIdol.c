...
void TrayParse( WPARAM t_wParam, LPARAM t_lParam )
{
	...
	else if ( t_lParam == WM_RBUTTONDOWN )
	{
		...
		AppendMenu( t_hTM, MF_STRING, WM_CREATE, TEXT( "Open Cursor ..." ) );
		AppendMenu( t_hTM, MF_SEPARATOR, 0, 0 );
		AppendMenu( t_hTM, MF_STRING, WM_DESTROY, TEXT( "Exit 2nd Hand" ) );

		GetCursorPos( &t_cPos );
		SetForegroundWindow( gm_hWnd );
		t_iTM = TrackPopupMenuEx( t_hTM, TPM_RETURNCMD | TPM_RIGHTALIGN | TPM_RIGHTBUTTON | TPM_BOTTOMALIGN | TPM_NONOTIFY | TPM_VERTICAL, t_cPos.x, t_cPos.y, gm_hWnd, 0 );
		DestroyMenu( t_hTM );
